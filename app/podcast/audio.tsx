...
export default function AudioPodcastPage() {
  ...
  async function loadPage() {
    try {
      ...
      setItems(rows.filter((item) => item.status === "published" && item.sourceType === "upload"));
    } catch (error: any) {
      Alert.alert("Chargement impossible", error?.message ?? "Impossible de charger les podcasts audio.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function openAudio(url: string | null) {
    if (!url) {
      Alert.alert("Indisponible", "Ce podcast audio n'a pas encore de lien lisible.");
      return;
    }
    const supported = await Linking.canOpenURL(url);
    if (!supported) {
      Alert.alert("Lecture impossible", "Impossible d'ouvrir ce podcast audio pour le moment.");
      return;
    }
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          ...
          <View style={styles.headerText}>
            <Text style={styles.title}>Podcasts audio</Text>
            <Text style={styles.subtitle}>Tous les podcasts audio publies dans un affichage compact.</Text>
          </View>
        </View>
        ...
        {loading ? (
          ...
          <View style={styles.emptyCard}>
            <Ionicons name="musical-notes-outline" size={22} color={COLORS.gray} />
            <Text style={styles.emptyTitle}>Aucun audio publie</Text>
            <Text style={styles.emptyText}>Les podcasts audio apparaitront ici quand ils seront disponibles.</Text>
          </View>
        ) : (
          <View style={styles.listWrap}>
            {items.map((item) => (
              <Pressable key={item.id ?? item.title} style={styles.itemCard} onPress={() => openAudio(item.audioUrl)}>
                ...
                <Ionicons name="play-circle-outline" size={20} color={COLORS.blueDark} />
              </Pressable>
            ))}
          </View>
        )}
