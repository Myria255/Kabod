...
export default function PodcastLivesPage() {
  ...
  async function loadPage() {
    try {
      ...
      setItems(rows.filter((item) => item.status === "scheduled" || item.status === "live" || item.status === "terminated"));
    } catch (error: any) {
  ...
  async function openLive(item: LiveStreamRecord) {
    const target = item.status === "live" || item.status === "scheduled"
      ? item.streamUrl ?? item.replayUrl
      : item.replayUrl ?? item.streamUrl;

    if (!target) {
      Alert.alert("Indisponible", "Aucun lien disponible pour ce live.");
      return;
    }

    const supported = await Linking.canOpenURL(target);
    if (!supported) {
      Alert.alert("Ouverture impossible", "Impossible d'ouvrir ce lien pour le moment.");
      return;
    }
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          ...
          <View style={styles.headerText}>
            ...
            <Text style={styles.subtitle}>Les directs programmes, en cours ou termines dans un affichage compact.</Text>
          </View>
        </View>
        ...
        {loading ? (
          ...
          <View style={styles.emptyCard}>
            <Ionicons name="radio-outline" size={22} color={COLORS.gray} />
            <Text style={styles.emptyTitle}>Aucun live visible</Text>
            <Text style={styles.emptyText}>Les lives programmes ou termines apparaitront ici.</Text>
          </View>
        ) : (
          <View style={styles.listWrap}>
            {items.map((item) => (
              <Pressable key={item.id ?? item.title} style={styles.itemCard} onPress={() => openLive(item)}>
                ...
                <View style={styles.itemBody}>
                  ...
                  <Text style={styles.itemMeta}>{formatDate(item.scheduledAt)} • {item.status}</Text>
                </View>
                <Ionicons name="arrow-forward-circle-outline" size={20} color={COLORS.blueDark} />
              </Pressable>
            ))}
          </View>
        )}
